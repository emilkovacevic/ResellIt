                    ...
                    <div className="max-w-sm mx-auto grid text-primary-foreground gap-6 md:grid-cols-2 lg:grid-cols-3 items-start md:max-w-2xl lg:max-w-none">
                        {categories.map((category) => (
                            <Link
                                href={`/${category.value}`}
                                key={category.value}
                                className="relative border cursor-pointer h-full flex flex-col items-center shadow p-6 bg-card rounded hover:bg-accent  hover:scale-105 transition-all duration-300"
                            >
